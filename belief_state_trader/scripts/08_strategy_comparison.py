"""Aggregate per-strategy backtest summaries into a single comparison artifact.

This script collects summary CSV files generated by earlier pipeline steps,
concatenates them into one table, enforces a consistent column order for core
metrics, and saves the combined result for downstream reporting/plotting.
...
If any required summary file is missing, the script raises a FileNotFoundError
to indicate that prerequisite scripts should be run first.
...
def format_for_display(df: pd.DataFrame) -> pd.DataFrame:
    """Return a display-friendly copy with percentages and key metrics formatted."""
    display = df.copy()
